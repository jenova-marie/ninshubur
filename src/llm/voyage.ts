...
/**
 * Retry budget for transient failures (429s, 5xx). With the free
 * tier's 3 RPM / 10K TPM limits we may hit 429 every other request;
 * with a payment method on file we'll only see them on rare bursts.
 */
const MAX_RETRIES = 6;
...
async function postEmbed(
  ...
  if (!env.VOYAGE_API_KEY) {
    throw new Error("VOYAGE_API_KEY is required for embedding commands.");
  }
  ...
  while (true) {
    ...
    if (retryable && attempt < MAX_RETRIES) {
      ...
      // capped at 60s. Free-tier limits reset on a 1-minute window.
...
export async function embed(opts: EmbedOptions): Promise<EmbedResult> {
  const model = opts.model ?? env.VOYAGE_MODEL;
  ...
  for (let offset = 0; offset < opts.texts.length; offset += MAX_BATCH) {
    ...
    const response = await postEmbed(slice, opts.inputType, model);
...
export async function embedQuery(text: string, model?: string): Promise<number[]> {
  const result = await embed({ texts: [text], inputType: "query", model });
